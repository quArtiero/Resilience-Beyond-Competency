"""
Module 2, Lesson 7: EI Mastery Capstone - 15-Day Plan with Metrics
"""

def get_lesson7_content():
    return {
        'id': 26,  # Lesson ID for Module 2, Lesson 7
        'module_number': 2,
        'order': 19,  # After Lesson 6 (order 18)
        'title': 'EI Mastery Capstone: 15-Day Plan with Metrics',
        'slug': 'ei-mastery-capstone',
        'description': 'Convert EI skills into automatic habits with a structured 15-day practice plan and measurable outcomes.',
        'is_published': True,
        'story': '''
...
# For database insertion if needed
if __name__ == "__main__":
    lesson = get_lesson7_content()
    print(f"Lesson {lesson['id']}: {lesson['title']}")
    print(f"Module: {lesson['module_number']}, Order: {lesson['order']}")
    print(f"Content length - Story: {len(lesson['story'])} chars")
    print(f"Content length - Reflection: {len(lesson['reflection'])} chars")
    print(f"Content length - Challenge: {len(lesson['challenge'])} chars")
